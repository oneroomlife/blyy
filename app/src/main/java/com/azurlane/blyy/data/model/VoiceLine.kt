...
data class VoiceLine(
    ...
    val scene: String,
    val dialogue: String,
    val audioUrlCn: String,
    val audioUrlJp: String
) {
    val audioUrl: String
        get() = audioUrlCn

    val hasDualAudio: Boolean
        get() = audioUrlCn.isNotEmpty() && audioUrlJp.isNotEmpty() && audioUrlCn != audioUrlJp

    fun getActiveAudioUrl(language: VoiceLanguage): String {
        return when (language) {
            VoiceLanguage.CN -> audioUrlCn.ifEmpty { audioUrlJp }
            VoiceLanguage.JP -> audioUrlJp.ifEmpty { audioUrlCn }
        }
    }
}
