...
- 🤖 **碧蓝航线小助手** *(v1.3.0 新增)*
  - 通过侧拉菜单进入小助手页面
  - **查玩家**：查询指挥官信息（等级、UID、收集率、资源、委托、科研进度、待办副本等）
  - **查建造**：查询玩家建造记录（支持分页，最多 500 条）
  - UID 与服务器在「设置 → 碧蓝航线助手」中统一配置，助手页面直接调用
...
- ⌚ **多端适配**
  - 完整支持手机、平板、可折叠设备
  - **小屏/手表端自适应** *(v1.3.0 新增)*：检测屏幕最小宽度 ≤ 360dp 时自动缩放 UI 元素
...
- 设置页增加「碧蓝航线助手」配置区域，集中管理 UID 与服务器
- AndroidManifest 新增 `<supports-screens>` 声明，明确支持手表等小屏设备
- 版本号升级至 1.3.0
